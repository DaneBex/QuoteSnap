...
4. customerMessage: 2 sentences, professional contractor tone. Sentence 1: "Thank you for the opportunity to provide an estimate for your [specific job type/description]." Sentence 2: summarize the specific scope in factual terms (materials, measurements, key steps included). AVOID: "transform", "beautiful new", "excited to work with you", "we'd love to", "we're confident", "excellent choice", "ready to move forward", "look forward to working with you", superlatives, sales copy, any phrasing that implies the customer has agreed to proceed or committed to anything. This is a quote, not a signed contract. Do NOT say details are missing if those details were already provided in the job notes or answers.
5. DAMAGE SCOPE — Never write definitive exclusions like "No mold remediation will be required." For mold, moisture, or hidden damage, always use conditional language: "Mold remediation is not included unless visible mold or moisture damage is discovered and separately approved." Apply the same principle to rot, structural damage, and concealed water intrusion.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: userRow } = await adminSupabase
      ...
      .eq("id", user.id)
      .single();
    type UserRow = { is_paid: boolean; beta_estimate_limit: number; total_estimates_created: number } | null;
    const row = userRow as UserRow;
    if (!(row?.is_paid ?? false)) {
      const betaLimit = row?.beta_estimate_limit ?? 3;
      const totalCreated = row?.total_estimates_created ?? 0;
